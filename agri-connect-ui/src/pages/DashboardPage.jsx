  ...
  return (
    ...
    >
      ...
      <div className="bg-white bg-opacity-80 max-w-4xl mx-auto mt-16 p-8 rounded shadow-md">
        <h1 className="text-3xl font-bold text-green-800 mb-6 text-center animate-fade-in-down">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {features.map((feature, index) => (
            <Link
              to={feature.route}
              key={feature.name}
              className={`block bg-white bg-opacity-90 p-6 rounded shadow-lg border border-green-200 hover:scale-105 transition-transform duration-300 animate-fade-in-up delay-${index * 100}`}
            >
              <h2 className="text-xl font-semibold text-green-700">{feature.name}</h2>
              <p className="text-sm text-gray-600 mt-2">
                Click to explore {feature.name}
              </p>
            </Link>
          ))}
        </div>
